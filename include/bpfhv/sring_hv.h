...
struct BpfhvState;
void *bpfhv_mem_map(struct BpfhvState *s,
                    hwaddr paddr, hwaddr *plen, int is_write);
void bpfhv_mem_unmap(struct BpfhvState *s, void *buffer,
                     hwaddr len, int is_write);

#if 0
typedef struct BpfhvImplOps {
    ...
    ssize_t (*sring_receive_iov)(struct BpfhvState *s,
    ...
    ssize_t (*txq_drain)(struct BpfhvState *s, NetClientState *nc,
...
ssize_t sring_txq_drain(struct BpfhvState *s, NetClientState *nc,
                        struct bpfhv_tx_context *ctx,
                        NetPacketSent *complete_cb, int vnet_hdr_len,
                        bool *notify);
